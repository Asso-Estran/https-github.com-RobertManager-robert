...
function montreDetailsPlan (datas) {
	...
	if (datas.matos != undefined) {
		$.each(datas.matos, function (titreSsCat, matosSsCat) {
			...
			$.each(matosSsCat, function(id, info) {
				detailMatosStr += '<div class="marge30l">'
									+'<span style="float:right">'+info.prix.toFixed(2)+' F</span>'
		...
		dialogContent += '<b>TOTAL : </b>'
						...
						+'<div class="rightText gros">H.T. : <span class="gras" id="totalHTplan">'+totalHT.toFixed(2)+'</span><b> F</b></div>'
						+'<div class="hide">RemisableHT : <span id="totalRemisablePlan">'+totalRemisableHT.toFixed(2)+'</span> F</div>'
						+'<div class="rightText gros">T.T.C. : <span class="gras" id="totalTTCplan">'+totalTTC.toFixed(2) +'</span><b> F</b></div>'
						+'<div class="rightText petit">Remisable : <span id="totalRemisablePlanTTC">'+totalRemisableTTC.toFixed(2)+'</span> F</div>'
						...
						+'</div><div class="rightText mini">'
							...
							+'donc <input type="text" id="remiseMontant" size="5" value="'+totalTTC.toFixed(2)+'" /> <b class="gros">F</b>'
						+'</div>'
						+'<div class="rightText enorme">'
							+'<span class="mini"><b>TOTAL T.T.C.</b> : </span><span class="gros gras" id="totalAPremise">'+totalTTC.toFixed(2)+'</span><b> F</b>'
						+'</div>'
						+'<div class="rightText petit marge15bot">'
							+'<span class="red" id="salairesTxt">Montant des salaires : </span><input type="text" class="NumericInput" size="5" onKeyUp="checkSalaires()" id="totalSalaires" /> F'
						+'</div>';
							...
							.append('<div style="float:right;" class="micro">'
									...
									+'<div class="inline top ui-state-highlight ui-corner-all pad3 mini" style="margin-top:2px;">'
										+'<a href="fct/downloader.php?dir=PlanDevis&planID='+idPlan+'&file='+encodeURIComponent(fileName)+'" title="CLIC pour télécharger">'
											+'<b>'+fileName+'</b>'+' <i class="petit">('+totalD+' F)</i>'
										+'</a>'
									+'</div>'
